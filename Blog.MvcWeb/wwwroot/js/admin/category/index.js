layui.use(['form', 'table', 'layer'], function () {
    ...
    table.on('tool(ID-table-demo-data)', function (obj) {
        ...
        switch (event) {
            case 'Edit':
                openDialog(data.blogCategoryId, event)
                break;
            case 'Del':
                layer.confirm(`是否删除该数据:标签${data.categoryName}?`, { icon: 3, title: 'tips' }, function (index) {
                    sendAjax("/Category/DeleteById", { blogCategoryId: data.blogCategoryId }, function (res) {
    ...
    let openDialog = function (id, action = 'View') {
        layer.open({
            type: 2,
            content: `/Admin/Category/AddOrEdit?id=${id}&action=${action}`, //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
            area: ['600px', '400px'],
            btn: ['确定', '取消'],
