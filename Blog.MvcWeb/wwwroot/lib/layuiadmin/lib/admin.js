...
layui.define('view', function(exports){
  ...
  var events = admin.events = {
    ...
    ,refresh: function () {
          ...
          var $tabBody = admin.tabsBody(admin.tabsPage.index);

          // 判断是 iframe 模式还是内容模式
          if ($tabBody.find('iframe').length > 0) {
              // iframe 模式 - 刷新 iframe
              var iframe = $tabBody.find(ELEM_IFRAME);
              ...
              $.ajax({
                  ...
                  success: function (html) {
                      // 提取主体内容（排除布局部分）
                      var $html = $(html);
                      var $newBody = $html.find('.layui-body .layadmin-tabsbody-item.layui-show').html();

                      if ($newBody) {
                          // 更新内容区域
                          $tabBody.html($newBody);
